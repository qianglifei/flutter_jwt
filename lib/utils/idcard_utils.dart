...
class IdCardUtils{
  ...
  static const List<int> _power = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2];

  int _currentYear = int.parse(DateFormat("yyyy").format(DateTime.now()));

  Map<String, String> _areaCode;

  static const List<String> _ValCodeArr = [
    "1",
    "0",
    "x",
    "9",
  ...
  static const int _YEAR_DELTA = 150;

  bool isIdCard(String idCard) {
    if (idCard == null || idCard.isEmpty) {
      Fluttertoast.showToast(msg: "身份证不能为空");
      return false;
    } else if (!(idCard.length == 15 || idCard.length == 18)) {
      Fluttertoast.showToast(msg: "身份证必须是15位或者18位");
      return false;
    }
    ...
    if (!_isDigital(ai)) {
      Fluttertoast.showToast(msg: "身份证处最后一位必须是纯数字");
      return false;
    }
    ...
    if (_dateRegExp.hasMatch(strYear + "-" + strMonth + "-" + strDay)) {
      Fluttertoast.showToast(msg: "出生年月日无效");
      return false;
    }
    ...
    if (yearDelta.abs() > _YEAR_DELTA) {
      Fluttertoast.showToast(msg: "年份不对，距离当前年份差量过大");
      return false;
    }
    if (int.parse(strMonth) > 12 || int.parse(strMonth) <= 0) {
      Fluttertoast.showToast(msg: "月份不对");
      return false;
    }
    if (int.parse(strDay) > 31 || int.parse(strDay) <= 0) {
       Fluttertoast.showToast(msg: "日期不对");
       return false;
    }
    if (!_getAreaCode().containsKey(ai.substring(0, 2))) {
      Fluttertoast.showToast(msg: "地区编码错误");
      return false;
    }
    int totalMulAiWi = 0;
    for (int i = 0; i < 17; i++) {
  ...
  int getAge(String idCard) {
    ...
    return AgeUtils().getAgeFromBirthday(dob);
  }
